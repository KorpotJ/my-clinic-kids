...
โปรเจกต์นี้สร้างด้วย AI เกือบทั้งหมด — วางแผนและออกแบบผ่านการคุยกับ Claude แล้วใช้ Claude Code เขียนโค้ดจริงในเครื่อง ส่วนที่ผมทำเองคือออกแบบหน้าแอป วางโครงสร้างระบบ และตัดสินใจว่าจะเอาอะไรไม่เอาอะไร ตั้งต้นจากโจทย์ที่ตัวเองตั้งไว้ว่าอยากสร้างของจริงบน AWS โดยใช้ LINE เป็นช่องทางหลักที่ผู้ปกครองเข้าถึง แล้วแบ่งงานออกเป็นชิ้นๆ ให้ AI ทำทีละส่วน
...
เพื่อหยุดค่าใช้จ่าย ผมลบอินสแตนซ์ RDS กับ VPC endpoint ของ Secrets Manager ออกไป ฐานข้อมูลเก็บไว้เป็น snapshot แบบ manual 5 ชุด ชุดหนึ่งถ่ายไว้ก่อนลบ ส่วน Lambda, API Gateway และ Cognito แทบไม่มีค่าใช้จ่าย เลยยัง deploy ทิ้งไว้

แผงควบคุมเจ้าหน้าที่รันอยู่บน CloudFront และยังเปิดดูได้อยู่แม้ฐานข้อมูลจะหายไปแล้ว ผมปิดมันเมื่อ 10 กันยายน 2569 เพราะไม่มีอะไรอยู่ข้างหลังมันอีกแล้ว และไม่อยากปล่อยหน้าแอดมินเปิดค้างไว้บนอินเทอร์เน็ตโดยไม่มีเหตุผล ผมเลือก disable ไม่ใช่ delete บัคเก็ต S3 กับโดเมนเดิมจึงยังอยู่ กดเปิดกลับได้ถ้าต้องใช้ การกู้ระบบทั้งหมดคือ restore snapshot แล้วสร้าง VPC endpoint ใหม่หนึ่งตัว
---
