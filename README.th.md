...
เพื่อไม่ให้มีค่าใช้จ่ายค้าง ผมลบ RDS instance กับ VPC endpoint ของ Secrets Manager ออก แล้วเก็บฐานข้อมูลไว้เป็น manual snapshot 5 ตัว รวมตัวที่ถ่ายทันทีก่อนลบด้วย ส่วนที่เหลือ — Lambda, API Gateway, Cognito, CloudFront, S3 — แทบไม่มีค่าใช้จ่าย เลยปล่อยไว้อย่างนั้น หน้าจอทั้งสองฝั่งจึงยังกดเข้าไปดูได้อยู่ เพียงแต่ข้อมูลจะไม่โหลดขึ้นมาแล้ว

ถ้าจะกู้ระบบกลับ ทำได้ด้วยการ restore snapshot แล้วสร้าง VPC endpoint ใหม่หนึ่งตัว
